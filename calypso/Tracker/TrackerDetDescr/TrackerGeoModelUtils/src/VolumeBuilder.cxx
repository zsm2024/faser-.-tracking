...
namespace TrackerDD {
  VolumeBuilder::VolumeBuilder(const Zone& zone, const std::vector<const ServiceVolume* >& services)
    : AthMessaging("TrackerDDVolumeBuilder"),
  ...
  VolumeBuilder::build(int iElement) {
    ...
    ATH_MSG_DEBUG("Volume/material: " << logName << "/" << materialName);
    if (!param.shapeType().empty()) ATH_MSG_DEBUG(" shape: " << param.shapeType());
    ATH_MSG_DEBUG(" volume (cm3): " << volume / Gaudi::Units::cm3);
    ATH_MSG_DEBUG(" rmin,rmax,zmin,zmax: "
                    << param.rmin() << ", "
                    << param.rmax() << ", "
                    << param.zmin() << ", "
                    << param.zmax());
