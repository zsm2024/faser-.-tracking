...
namespace G4UA
{
  ...
  //---------------------------------------------------------------------------
  FaserLooperKiller::FaserLooperKiller(const Config& config)
    : AthMessaging(Gaudi::svcLocator()->service<IMessageSvc>("MessageSvc"),
                   "FaserLooperKiller"),
      m_evtStore("StoreGateSvc/StoreGateSvc", "FaserLooperKiller"),
      m_detStore("StoreGateSvc/DetectorStore", "FaserLooperKiller"),
      m_config(config), m_report(), m_count_steps(0)
  {
  }

  //---------------------------------------------------------------------------
  void FaserLooperKiller::UserSteppingAction(const G4Step* aStep)
  {
    ...
    if (aStep->GetTrack()->GetCurrentStepNumber() < m_config.MaxSteps) {
      ...
      ATH_MSG_WARNING("FaserLooperKiller triggered!! Hold on to your hats!!!!!!!!" );
    }
