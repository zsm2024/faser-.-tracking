...
FaserGeo2G4Builder::FaserGeo2G4Builder(std::string detectorName)
  : AthMessaging(detectorName)
  ...
  , m_pDetStore(nullptr)
{
  ...
  if (sc.isFailure()) {
    ATH_MSG_FATAL("FaserGeo2G4Builder for detector "<<detectorName<<"could not access the detector store - PANIC!!!!");
    abort();
  }

  const GeoModelExperiment* theExpt = nullptr;
  ATH_MSG_ALWAYS("Calling retrieve");
  sc = m_pDetStore->retrieve( theExpt, "FASER" );
  if(sc.isFailure()){
    ...
    {
        // -------- -------- MATERIAL MANAGER -------- ----------
        const StoredMaterialManager* theMaterialManager = m_pDetStore->tryRetrieve<StoredMaterialManager>("MATERIALS");
    ...
    FaserGeo2G4SvcAccessor accessor;
    FaserGeo2G4SvcBase *g=accessor.GetGeo2G4Svc();
    ...
    if(m_theBuilder)
      ...
      ATH_MSG_WARNING("0 pointer to volume builder."
                      ...
                      <<"\n 'GetVolumeBuilder' method of FaserGeo2G4Builder");
  }
}

G4LogicalVolume* FaserGeo2G4Builder::BuildTree()
{
  ATH_MSG_DEBUG("Entering FaserGeo2G4Builder::BuildTree()...");
  ...
  if(m_theBuilder) {
    if(m_treeTops.size()==1) {
      ...
      StatusCode sc = m_pDetStore->retrieve(theExpt);
      if(sc.isFailure())
...
FaserVolumeBuilder*  FaserGeo2G4Builder::GetVolumeBuilder(std::string bname)
{
  FaserGeo2G4SvcAccessor accessor;
  FaserGeo2G4SvcBase *g=accessor.GetGeo2G4Svc();
...
void FaserGeo2G4Builder::BuildOpticalSurfaces(const GeoBorderSurfaceContainer* surface_container,
                                         const OpticalVolumesMap* optical_volumes)
{
  FaserGeo2G4OpticalSurfaceFactory surfaceFactory;
