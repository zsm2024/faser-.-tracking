...
//
// The FaserG4ShiftedCone class copied from standard G4Cons and modified to:
...
//
//
// class FaserG4ShiftedCone
//
// Implementation for FaserG4ShiftedCone class
//
// History:
// 03.07.2019 A. Sukharev: copied from G4Cons
// --------------------------------------------------------------------

#include "FaserG4ShiftedCone.h"
...
FaserG4ShiftedCone::FaserG4ShiftedCone( const G4String& pName,
    ...
    // fSPhi(0.), fDPhi(0.)
{
  ...
  if ( fDz < 0 )
  {
    ...
    message << "Invalid Z half-length for Solid: " << GetName() << G4endl
            << "        hZ = " << fDz;
    G4Exception("FaserG4ShiftedCone::FaserG4ShiftedCone()", "GeomSolids0002",
...
//
FaserG4ShiftedCone::FaserG4ShiftedCone( __void__& a )
...
FaserG4ShiftedCone::~FaserG4ShiftedCone()
{
}
...
FaserG4ShiftedCone::FaserG4ShiftedCone(const FaserG4ShiftedCone& rhs)
...
FaserG4ShiftedCone& FaserG4ShiftedCone::operator = (const FaserG4ShiftedCone& rhs)
{
...
EInside FaserG4ShiftedCone::Inside(const G4ThreeVector& p) const
{
...
void FaserG4ShiftedCone::ComputeDimensions(      G4VPVParameterisation*,// p,
                               const G4int                  ,//n,
                               const G4VPhysicalVolume*     )//pRep    )
{
    std::ostringstream message;
    message << "ComputeDimensions is not implemented for Solid: " << GetName();
    G4Exception("FaserG4ShiftedCone::ComputeDimensions()", "GeomSolids0002",
                FatalException, message) ;
...
void FaserG4ShiftedCone::BoundingLimits(G4ThreeVector& pMin, G4ThreeVector& pMax) const
{
...
/*  if (GetDeltaPhiAngle() < twopi)
  ...
  if (pMin.x() >= pMax.x() || pMin.y() >= pMax.y() || pMin.z() >= pMax.z())
  {
    ...
    message << "Bad bounding box (min >= max) for solid: "
            ...
            << "\npMin = " << pMin
            << "\npMax = " << pMax;
    G4Exception("FaserG4ShiftedCone::BoundingLimits()", "GeomMgt0001",
                JustWarning, message);
    DumpInfo();
  }
}
...
G4bool FaserG4ShiftedCone::CalculateExtent( const EAxis              pAxis,
...
G4ThreeVector FaserG4ShiftedCone::SurfaceNormal( const G4ThreeVector& p) const
{
  G4int noSurfaces = 0;
  G4double rho;//, pPhi;
...
/*  if( !fPhiFullCone )
  ...
  {
#ifdef G4CSGDEBUG
    G4Exception("FaserG4ShiftedCone::SurfaceNormal(p)", "GeomSolids1002",
...
G4ThreeVector FaserG4ShiftedCone::ApproxSurfaceNormal( const G4ThreeVector& p ) const
{
  ENorm side ;
  G4ThreeVector norm ;
  G4double rho;//, phi ;
...
//      break ;
    default:          // Should never reach this case...
      DumpInfo();
      G4Exception("FaserG4ShiftedCone::ApproxSurfaceNormal()",
...
G4double FaserG4ShiftedCone::DistanceToIn( const G4ThreeVector& p,
...
/*            if ( fPhiFullCone )  { return sd; }
    ...
    {
      // Inside outer cone
      // check not inside, and heading through FaserG4ShiftedCone (-> 0 to in)
...
G4double FaserG4ShiftedCone::DistanceToIn(const G4ThreeVector& p) const
{
...
G4double FaserG4ShiftedCone::DistanceToOut( const G4ThreeVector& p,
...
/*      case kSPhi:
      ...
      default:
        ...
        G4Exception("FaserG4ShiftedCone::DistanceToOut(p,v,..)","GeomSolids1002",
                    JustWarning, message) ;
        break ;
    }
  }
...
G4double FaserG4ShiftedCone::DistanceToOut(const G4ThreeVector& p) const
{
  ...
  if( Inside(p) == kOutside )
  {
    ...
    G4Exception("FaserG4ShiftedCone::DistanceToOut(p)", "GeomSolids1002",
...
/*
  if (!fPhiFullCone)
  {
    // Above/below central phi of FaserG4ShiftedCone?
...
G4GeometryType FaserG4ShiftedCone::GetEntityType() const
{
  return G4String("FaserG4ShiftedCone");
}
...
//
G4VSolid* FaserG4ShiftedCone::Clone() const
{
  return new FaserG4ShiftedCone(*this);
}
...
std::ostream& FaserG4ShiftedCone::StreamInfo(std::ostream& os) const
{
...
G4ThreeVector FaserG4ShiftedCone::GetPointOnSurface() const
{
...
void FaserG4ShiftedCone::DescribeYourselfTo (G4VGraphicsScene& scene) const
{
  scene.AddSolid (*this);
}

G4Polyhedron* FaserG4ShiftedCone::CreatePolyhedron () const
{
