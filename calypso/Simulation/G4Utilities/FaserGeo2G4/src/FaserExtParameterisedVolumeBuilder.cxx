...
#include "FaserGeo2G4LVFactory.h"
...
FaserExtParameterisedVolumeBuilder::FaserExtParameterisedVolumeBuilder(std::string n, FaserGeo2G4AssemblyFactory* G4AssemblyFactory):
  FaserVolumeBuilder(n),
  AthMessaging(n),
  m_getMatEther(true),
  m_matEther(0),
  m_matHypUr(0),
...
G4LogicalVolume* FaserExtParameterisedVolumeBuilder::Build(const PVConstLink theGeoPhysVolume, OpticalVolumesMap* optical_volumes) const
{
  ...
  static FaserGeo2G4LVFactory LVFactory;
  ...
  if(serialExists && (numChildNodes==1))
    {
      ...
      FaserGeo2G4STParameterisation* stParameterisation = new FaserGeo2G4STParameterisation(serialTransformerChild->getFunction(),
      ...
      while (!av.atEnd())
        {
          ...
          if(m_matEther == theGeoPhysChild->getLogVol()->getMaterial())
            {
              FaserGeo2G4AssemblyVolume* assembly = BuildAssembly(theGeoPhysChild);
...
FaserGeo2G4AssemblyVolume* FaserExtParameterisedVolumeBuilder::BuildAssembly(PVConstLink pv) const
{
  PVConstLink theGeoPhysChild;
  G4LogicalVolume* theG4LogChild = 0;
  FaserGeo2G4AssemblyVolume* theG4AssemblyChild = 0;
  bool descend;                                    // flag to continue geo tree navigation

  if(m_getMatEther) getMatEther();

  FaserGeo2G4AssemblyVolume* assemblyVolume = m_G4AssemblyFactory->Build(pv,descend);
...
void FaserExtParameterisedVolumeBuilder::PrintSTInfo(const std::string& volume) const
{
  ATH_MSG_INFO ( "**********************************************" );
  ATH_MSG_INFO ( "**  " );
...
void FaserExtParameterisedVolumeBuilder::getMatEther() const
{
  StoreGateSvc* pDetStore=0;
  ISvcLocator* svcLocator = Gaudi::svcLocator();
  if(svcLocator->service("DetectorStore",pDetStore).isFailure()) {
    ATH_MSG_ERROR ( "FaserExtParameterisedVolumeBuilder: Unable to access Detector Store" );
  }
  else
    {
      const StoredMaterialManager* theMaterialManager = nullptr;
      if(pDetStore->retrieve(theMaterialManager, "MATERIALS").isFailure()) {
        ATH_MSG_ERROR ( "FaserExtParameterisedVolumeBuilder: Unable to access Material Manager" );
      }
      else {
