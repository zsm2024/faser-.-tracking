...
{ 
  ...
  msg(MSG::DEBUG) << "Created VetoDetectorManager" << endmsg;

  // Create the database
  m_db = new VetoDataBase{athenaComps};
  msg(MSG::DEBUG) << "Created VetoDataBase" << endmsg;

  // Create the material manager
  m_materials = new VetoMaterialManager{m_db};
  msg(MSG::DEBUG) << "Created VetoMaterialManager" << endmsg;

  // Create the geometry manager.
  m_geometryManager = new VetoGeometryManager{m_db};
  msg(MSG::DEBUG) << "Created VetoGeometryManager" << endmsg;
  m_geometryManager->setOptions(options);
  msg(MSG::DEBUG) << "Set options on VetoGeometryManager" << endmsg;

  m_useDynamicAlignFolders = options.dynamicAlignFolders();
 
  // Set Version information
  // Get the geometry tag
  DecodeFaserVersionKey versionKey(geoDbTagSvc(),"Veto");
  IRDBRecordset_ptr switchSet
    = rdbAccessSvc()->getRecordsetPtr("VetoSwitches", versionKey.tag(), versionKey.node(),"FASERDD");
  const IRDBRecord    *switches   = (*switchSet)[0];
  msg(MSG::DEBUG) << "Retrieved VetoSwitches" << endmsg;
  ...
  std::string versionTag = rdbAccessSvc()->getChildTag("Veto", versionKey.tag(), versionKey.node(),"FASERDD");
...
{ 
  ...
  if (stationB_Present)
  {
      msg(MSG::DEBUG) << "Building the Veto Station B." << endmsg;
  ...
  const ScintDD::ScintDetectorDesign* design = m_detectorManager->getVetoDesign();
  if (design != nullptr)
  {
