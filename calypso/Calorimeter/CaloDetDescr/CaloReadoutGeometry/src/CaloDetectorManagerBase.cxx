...
namespace CaloDD 
{
  ...
  // Destructor
    CaloDetectorManagerBase::~CaloDetectorManagerBase() 
        {}


    const Version& CaloDetectorManagerBase::getVersion() const
    {
        return m_version;
    }

    const std::string& CaloDetectorManagerBase::getLayout() const
    {
        return m_version.layout();
    }

    void CaloDetectorManagerBase::setVersion(const Version & version)
    {
        m_version = version;
    }

    void CaloDetectorManagerBase::addChannel(const std::string & key, int level, FrameType frame)
    {
        std::string frameStr = "other";
        if (frame == CaloDD::global) frameStr = "global";
        if (frame == CaloDD::local) frameStr  = "local";
        ATH_MSG_INFO("Registering alignment channel with key " << key << ", level " << level 
                     << ", with frame " << frameStr << ".");
        m_keys[key] = LevelInfo(level, frame); 
    }

    void CaloDetectorManagerBase::addFolder(const std::string & key)
    {
        m_folders.insert(key);
    }

    void CaloDetectorManagerBase::addSpecialFolder(const std::string & key)
    {
        m_specialFolders.insert(key);
    }

    void CaloDetectorManagerBase::addGlobalFolder(const std::string & key)
    {
        m_globalFolders.insert(key);
    }

    void CaloDetectorManagerBase::addAlignFolderType(const AlignFolderType  alignfolder)
    {
        m_alignfoldertype = alignfolder;
    }

  // Return the level in the hierarchy (user defined) corresponding to the key.
    const CaloDetectorManagerBase::LevelInfo& CaloDetectorManagerBase::getLevel(const std::string & key) const 
    {
    ...
    StatusCode CaloDetectorManagerBase::align( IOVSVC_CALLBACK_ARGS_P(I,keys) ) const
    {
    ...
    StatusCode CaloDetectorManagerBase::align(const RawAlignmentObjects& alignObjects, GeoVAlignmentStore* alignStore) const
    {
    ...
    bool CaloDetectorManagerBase::processAlignmentContainer(const std::string & key) const
    {
        bool alignmentChange = false;
        ...
        if (StatusCode::SUCCESS!=m_detStore->retrieve(container, key)) {        
            ...
            throw std::runtime_error("Unable to apply Calorimeter alignments");
        }
    ...
    bool CaloDetectorManagerBase::processAlignmentContainer(const AlignableTransformContainer* container, GeoVAlignmentStore* alignStore) const
    {
        bool alignmentChange = false;
    ...
    bool CaloDetectorManagerBase::processKey(const std::string key,
  ...
  // We provide a default implementation of any detector specific alignment.
    bool CaloDetectorManagerBase::processGlobalAlignmentContainer(const std::string & key,
                                                               const CondAttrListCollection* obj,
                                                               GeoVAlignmentStore* alignStore) const
    {
      bool alignmentChange = false;
  ...
  // We provide a default implementation of any detector specific alignment.                                                                                 
    bool CaloDetectorManagerBase::processGlobalAlignment(const std::string &, int /*level*/, FrameType /*frame*/,
  ...
  // We provide a default implementation of any detector specific alignment.
    bool CaloDetectorManagerBase::processSpecialAlignment(const std::string &, CaloDD::AlignFolderType) const
    {
        return false;
    }

    const CaloDetectorManagerBase::LevelInfo CaloDetectorManagerBase::s_invalidLevel;
