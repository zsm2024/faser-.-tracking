...
/**
 * @file CaloDetectorElement.cxx
 * Implementation file for class CaloDetectorElement
...
CaloDetectorElement::CaloDetectorElement(const Identifier &id,
...
void
CaloDetectorElement::commonConstructor() 
{
  if (!m_id.is_valid()) throw std::runtime_error("CaloDetectorElement: Invalid identifier");

  m_isEcal = getIdHelper()->is_ecal(m_id);
  if (!m_isEcal)
  {
    if (msgLvl(MSG::WARNING)) msg(MSG::WARNING) << "Element id is not for Ecal" << endmsg;
  }
  
  // Set IdHash.
  if (isEcal())
  {
      const EcalID* ecalId = dynamic_cast<const EcalID* >(getIdHelper());
      m_idHash = ecalId->module_hash(m_id);
  }
  
  if (!m_idHash.is_valid()) throw std::runtime_error("CaloDetectorElement: Unable to set IdentifierHash");

  // Increase the reference count of the CaloDetectorDesign objects.
  m_design->ref();

  // Increase the reference count of the CaloCommonItems objects.
  m_commonItems->ref();
...
// Destructor:
CaloDetectorElement::~CaloDetectorElement()
{
...
void 
CaloDetectorElement::updateCache() const
{
  ...
  // This is accomplished simply by the fixed initialization of m_hitEta, m_hitPhi and m_hitDepth in CaloDetectorDesign
  //
...
CaloDetectorElement::transformHit() const
{
  if (m_geoAlignStore) {
...
CaloDetectorElement::transform() const
{
  if (!m_cacheValid){                                                                                               
    std::lock_guard<std::recursive_mutex> lock(m_mutex);                                                            
...
CaloDetectorElement::transformCLHEP() const
{
...
const HepGeom::Transform3D 
CaloDetectorElement::defTransformCLHEP() const
{
  if (m_geoAlignStore) {
...
const Amg::Transform3D 
CaloDetectorElement::defTransform() const
{
...
const HepGeom::Transform3D 
CaloDetectorElement::recoToHitTransform() const
{
...
const Amg::Transform3D &
CaloDetectorElement::moduleTransform() const
{
  return transform();
  ...
  Amg::Transform3D
  CaloDetectorElement::defModuleTransform() const
{
  return defTransform();
...
Amg::Transform3D 
CaloDetectorElement::localToModuleFrame(const Amg::Transform3D & localTransform) const
{
  // if (isModuleFrame()) {
...
Amg::Transform3D 
CaloDetectorElement::localToModuleTransform() const
{
  // if (isModuleFrame()) {
...
// bool 
// CaloDetectorElement::isModuleFrame() const
// {
...
const Amg::Vector3D & 
CaloDetectorElement::center() const
{
  if (!m_cacheValid){                                                                                               
    std::lock_guard<std::recursive_mutex> lock(m_mutex);                                                            
    if (!m_cacheValid) updateCache();                                                                               
...
const Amg::Vector3D & 
CaloDetectorElement::normal() const
{
  if (!m_cacheValid){                                                                                               
    std::lock_guard<std::recursive_mutex> lock(m_mutex);                                                            
    if (!m_cacheValid) updateCache();                                                                               
...
CaloDetectorElement::etaAxisCLHEP() const
{
  if (!m_cacheValid){                                                                                               
    std::lock_guard<std::recursive_mutex> lock(m_mutex);                                                            
    if (!m_cacheValid) updateCache();                                                                               
...
const HepGeom::Vector3D<double> & 
CaloDetectorElement::phiAxisCLHEP() const
{
  if (!m_cacheValid){                                                                                               
    std::lock_guard<std::recursive_mutex> lock(m_mutex);                                                            
    if (!m_cacheValid) updateCache();                                                                               
...
const Amg::Vector3D & 
CaloDetectorElement::etaAxis() const
{
  if (!m_cacheValid){                                                                                               
    std::lock_guard<std::recursive_mutex> lock(m_mutex);                                                            
    if (!m_cacheValid) updateCache();                                                                               
...
const Amg::Vector3D & 
CaloDetectorElement::phiAxis() const
{
  if (!m_cacheValid){                                                                                               
    std::lock_guard<std::recursive_mutex> lock(m_mutex);                                                            
    if (!m_cacheValid) updateCache();                                                                               
...
Amg::Vector2D
CaloDetectorElement::hitLocalToLocal(double xEta, double xPhi) const  // Will change order to phi,eta
{
  std::lock_guard<std::recursive_mutex> lock(m_mutex);
  if (!m_cacheValid) updateCache();
...
CaloDetectorElement::hitLocalToLocal3D(const HepGeom::Point3D<double> & hitPosition) const
{
...
bool CaloDetectorElement::isEcal() const
{
  return m_isEcal;
}

Trk::Surface & 
CaloDetectorElement::surface()
{
...
const Trk::Surface & 
CaloDetectorElement::surface() const
{
  if (not m_surface) m_surface.set(std::make_unique<Trk::PlaneSurface>(*this));
  return *m_surface;
}
  
const std::vector<const Trk::Surface*>& CaloDetectorElement::surfaces() const 
{
    std::lock_guard<std::recursive_mutex> lock(m_mutex);
    if (!m_surfaces.size()) {
        // get this surface
...
CaloDetectorElement::bounds() const
{
  return m_design->bounds();
}
  
// Get min/max or r, z,and phi
void CaloDetectorElement::getExtent(double &xMin, double &xMax, 
...
// void CaloDetectorElement::getEtaPhiRegion(double deltaZ, double &etaMin, double &etaMax, double &phiMin, 
...
// void CaloDetectorElement::getEtaPhiPoint(const HepGeom::Point3D<double> & point, double deltaZ, 
...
void CaloDetectorElement::getCorners(HepGeom::Point3D<double> *corners) const
{
  std::lock_guard<std::recursive_mutex> lock(m_mutex);
  if (!m_cacheValid) updateCache();
...
CaloIntersect
CaloDetectorElement::inDetector(const Amg::Vector2D & localPosition, 
...
CaloIntersect 
CaloDetectorElement::inDetector(const HepGeom::Point3D<double> & globalPosition, double phiTol, double etaTol) const
{
  return m_design->inDetector(localPosition(globalPosition), phiTol, etaTol);
}

// bool 
// CaloDetectorElement::nearBondGap(Amg::Vector2D localPosition, double etaTol) const
// {
//   return m_design->nearBondGap(localPosition, etaTol);
// }

// bool
// CaloDetectorElement::nearBondGap(HepGeom::Point3D<double> globalPosition, double etaTol) const
// {
//   return m_design->nearBondGap(localPosition(globalPosition), etaTol);
// }  

// Amg::Vector2D
// CaloDetectorElement::rawLocalPositionOfCell(const SiCellId &cellId) const
// {
//   return m_design->localPositionOfCell(cellId);
// }

// Amg::Vector2D
// CaloDetectorElement::rawLocalPositionOfCell(const Identifier & id) const
// {
...
// int 
// CaloDetectorElement::numberOfConnectedCells(const SiCellId cellId) const
// {
...
// SiCellId 
// CaloDetectorElement::connectedCell(const SiCellId cellId, int number) const
// {
//   SiReadoutCellId readoutId = m_design->readoutIdOfCell(cellId);
...
// SiCellId 
// CaloDetectorElement::cellIdOfPosition(const Amg::Vector2D &localPosition) const
// {
//   return m_design->cellIdOfPosition(localPosition);
// }

// Identifier
// CaloDetectorElement::identifierOfPosition(const Amg::Vector2D &localPosition) const
// {
...
// Identifier 
// CaloDetectorElement::identifierFromCellId(const SiCellId & cellId) const
// {
...
// SiCellId   
// CaloDetectorElement::cellIdFromIdentifier(const Identifier & identifier) const
// {
