...
void
CaloHitCnv_p1::persToTrans(const CaloHit_p1* persObj, CaloHit* transObj, MsgStream &log)
{
  HepMcParticleLinkCnv_p2 HepMcPLCnv;
  HepMcParticleLink link;
  HepMcPLCnv.persToTrans(&(persObj->m_partLink),&link, log);

   *transObj = CaloHit (HepGeom::Point3D<double> (persObj->m_stX,
                                                   persObj->m_stY,
                                                   persObj->m_stZ),
...
void
CaloHitCnv_p1::transToPers(const CaloHit* transObj, CaloHit_p1* persObj, MsgStream &log)
{
//     if (log.level() <= MSG::DEBUG) log << MSG::DEBUG << "CaloHitCnv_p1::transToPers called " << endmsg;
   HepMcParticleLinkCnv_p2 HepMcPLCnv;
