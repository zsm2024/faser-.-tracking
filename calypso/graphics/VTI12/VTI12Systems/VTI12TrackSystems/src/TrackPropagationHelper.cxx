  ...
  if (volume && trackParam && !m_d->outsideIDVolume(trackParam->position())) {
    ...
    if (bsurfs){
      ...
      for (;bSurfsIt!= bsurfs->end(); bSurfsIt++){
        ...
        const Trk::TrackParameters*  trackPar = extrapolator->extrapolate(ctx,*trackParam,**bSurfsIt,Trk::alongMomentum,true,hypo).release(); // change this to extrapolate current param to surface.
                         ...
                         const double& dist )
{
  ...
  try {
    ...
    newpars = extrapolator->extrapolate(Gaudi::Hive::currentContext(),*prevpars,surf,Trk::alongMomentum,false,hypo).release(); // change this to extrapolate current param to surface.
