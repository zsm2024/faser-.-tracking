...
public:
  ...
  static Trk::Perigee * createTrkPerigeeFromProdVertex(HepMC::ConstGenParticlePtr p, const double& charge )
  {
    ...
    if (!v)
      return 0;//Fixme: message!
    // std::cout << "Creating truth track prod vertex at: " << v->point3d().x() << " " << v->point3d().y() << " " << v->point3d().z() << std::endl;
  ...
  static Trk::TrackParameters * createTrkParamFromDecayVertex(HepMC::ConstGenParticlePtr p, const double& charge )
  {
    ...
    if (!v)
      return 0;//Fixme: message!
    // std::cout << "Creating truth track end vertex at: " << v->point3d().x() << " " << v->point3d().y() << " " << v->point3d().z() << std::endl;
    Amg::Vector3D mom(p->momentum().px(),p->momentum().py(),p->momentum().pz());
//     double absmom(mom.mag());
//     if (absmom<=0)
//       return 0;//Fixme: message!
...
void TrackHandle_TruthTrack::Imp::ensureInitTrkTracks()
{
  ...
  // std::cout << "TruthTrack useGen, decayedGen, useSim = " << useGenParticle << decayedGenParticle << useSimHits << std::endl;
...
void TrackHandle_TruthTrack::Imp::ensureInitAscObjs()
{
  ...
  HepMC::ConstGenVertexPtr vprod = genParticle ? genParticle->production_vertex() : 0;
  HepMC::ConstGenVertexPtr vend = genParticle ? genParticle->end_vertex() : 0;
