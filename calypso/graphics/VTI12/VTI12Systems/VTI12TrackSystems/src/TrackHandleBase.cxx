...
#include "VTI12TrackSystems/TrackCollHandleBase.h"
#include "VTI12TrackSystems/TrackSystemController.h"
#include "VTI12TrackSystems/VP1TrackSystem.h"
#include "VTI12TrackSystems/TrackPropagationHelper.h"
#include "VTI12TrackSystems/TrackLODManager.h"
#include "VTI12TrackSystems/AssociatedObjectHandleBase.h"
#include "VTI12TrackSystems/AscObjSelectionManager.h"
#include "VTI12TrackSystems/VP1TrackSanity.h"
#include "VTI12TrackSystems/SimBarCode.h"//For unknown pdg. Fixme: Use 0 for unknown pdg!
...
#include "VTI12Utils/SurfaceToSoNode.h"
...
// Add labels
  if (!points->empty() && theclass->common()->controller()->doTrackLabels() ){
    ...
    if (labels&TrackSystemController::Hits){
      text <<"SCT["+QString::number(theclass->getNSCTHits())+"]";
    }
      ...
      if (labels&TrackSystemController::Direction ){
        ...
        if (mom.mag2()==0.0) {
          ...
          double polarAngle = mom.theta();
          text << VP1Msg::str("(theta,phi)=(")
            +VP1Msg::str(polarAngle)+VP1Msg::str(",")+VP1Msg::str(VP1LinAlgUtils::phiFromXY(mom.x(), mom.y() ))+VP1Msg::str(")");
        }
      }
  ...
  if (pathInfo_TrkTrack) {
    ...
    if (points_raw->size()==1) {
      ...
      if (firstmomentum->mag()==0.0) {
        ...
        points_raw->push_back(points_raw->front()+(*firstmomentum)*(1000.0/firstmomentum->mag()));
      }
...
Trk::ParticleHypothesis TrackHandleBase::extrapolationParticleHypothesis() const
{
  switch(abs(pdgCode())) {
    ...
    case 0:
    default:
    return ((hasCharge()&&charge()==0.0) ? Trk::nonInteracting : Trk::muon);
  }
}
...
QStringList TrackHandleBase::baseInfo() const
{
  ...
  if (mom.mag2()==0.0) {
    ...
    l << VP1Msg::str("(")+QChar(0x03B8)+","+QChar(0x03D5)+VP1Msg::str(")=(")
        +VP1Msg::str(mom.theta())+VP1Msg::str(",")+VP1Msg::str(VP1LinAlgUtils::phiFromXY(mom.x(), mom.y() ))+VP1Msg::str(")");
    
    l << "Theta: "+VP1Msg::str(mom.theta());
    l << "Phi: "+VP1Msg::str(VP1LinAlgUtils::phiFromXY(mom.x(), mom.y() ));
    l << "ThetaXZ: "+VP1Msg::str(atan2(mom.x(), mom.z() ));
    l << "ThetaYZ: "+VP1Msg::str(atan2(mom.y(), mom.z() ));
  }
  l<<"Hit summary: SCT["+QString::number(getNSCTHits())+"].";
  ...
  if (m_d->pathInfo_TrkTrack) {
    ...
    for (; tsos_iter != tsos_end; ++tsos_iter) {
      if (common()->trackSanityHelper()->isSafe(*tsos_iter)) {
        ...
        if (common()->trackSanityHelper()->isSafe(trackParam))
        {
          // std::cout << "endPoint returning (1): " << trackParam->position() << std::endl;
          return trackParam->position();
        }
      }
    }
  } else if (m_d->pathInfo_Points&&!m_d->pathInfo_Points->empty()) {
    // std::cout << "endPoint returning (2): " << m_d->pathInfo_Points->back() << std::endl;
    return m_d->pathInfo_Points->back();
  }
  // std::cout << "endPoint returning (3) (null position)" << std::endl;
  return {};
}
